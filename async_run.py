...
async def async_check_network_health_alerts(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
    ...
    try:
        ...
        pp(f'[bold magenta]Meraki AIO API Error (OrgID "{ org_id }", OrgName "{ org_name }"): \n { e }')
    except Exception as e:
...
async def async_check_wifi_channel_utilization(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
    ...
    result = {'is_ok': True}
    try:
        channel_utilization = await aiomeraki.networks.getNetworkNetworkHealthChannelUtilization(network['id'], perPage=100)
        # TODO: pagination
        for ap in channel_utilization:
            ...
            if len(utilization_list) == 0:
                ...
                result[ap['serial']] = {'is_ok': True, 'utilization': max(utilization_list), 'occurances': 0}
        # Adding AP names
        try:
            network_devices = await aiomeraki.networks.getNetworkDevices(network['id'])
        except meraki.exceptions.AsyncAPIError as e:
            pp(f'[bold magenta]Meraki AIO API Error (Network Name "{network["name"]}": \n { e }')
        except Exception as e:
            pp(f'[bold magenta]Some other ERROR: {e}')    
...
async def async_check_wifi_rf_profiles(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
    ...
    result = {'is_ok': True}
    try:
        rf_profiles = await aiomeraki.wireless.getNetworkWirelessRfProfiles(network['id'])
    except:
...
async def async_check_wifi_ssid_amount(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
    ...
    if enabled_ssid_counter <= thresholds['ssid_amount']:
        pp(f"[green]There are {enabled_ssid_counter} SSIDs enabled for network {network['name']}")
    else:
        pp(f"[red]There are {enabled_ssid_counter} SSIDs enabled for network {network['name']}")
        result['is_ok'] = False
    results[network['name']]['ssid_amount_check'] = result


async def async_check_switches_port_counters(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
    ...
    for device in device_list:
        if "MS" in device['model']:
            ...
            port_check_task.append(async_check_switch_port_counters(aiomeraki, network, device))
            results[network['name']]['port_counters_check'][device['name']] = {'is_ok': True}
    #
    for task in asyncio.as_completed(port_check_task):
            await task


async def async_check_switch_port_counters(aiomeraki: meraki.aio.AsyncDashboardAPI, network:dict, device: dict):
    #print(f"\t\tChecking Switch Port Counters for network {network['name']}...")
    result = {'is_ok': True, 'crc': [], 'collision': [], 'broadcast': [], 'multicast': [], 'topology_changes': []}
    try:
        switch_counters = await aiomeraki.switch.getDeviceSwitchPortsStatusesPackets(device['serial'])
    except meraki.exceptions.AsyncAPIError as e:
        pp(f'[bold magenta]Meraki AIO API Error (OrgID "{ org_id }", OrgName "{ org_name }"): \n { e }')
    except Exception as e:
        pp(f'[bold magenta]Some other ERROR: {e}') 
    for port in switch_counters:
        for port_counter in port['packets']:
            ...
            if port_counter['desc'] == "Broadcast" and port_counter['ratePerSec']['total'] > thresholds['broadcast_rate']:
                ...
                result['is_ok'] = False
            if port_counter['desc'] == "Multicast" and port_counter['ratePerSec']['total'] > thresholds['multicast_rate']:
                ...
                result['is_ok'] = False
            # Topology changes
            if port_counter['desc'] == "Topology changes" and port_counter['total'] > thresholds['topology_changes']:
...
async def async_check_switch_stp(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
...
async def async_check_switch_mtu(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
    ...
    if mtu['defaultMtuSize'] == 9578 and mtu['overrides'] == []:
        pp(f"[green]Jumbo Frames enabled for network {network['name']} (MTU: {mtu['defaultMtuSize']})")
        results[network['name']]['mtu_check'] = {'is_ok': True}
    else:
        pp(f"[red]Jumbo Frames disabled for network {network['name']} (MTU: {mtu['defaultMtuSize']}).\
...
async def async_check_switch_storm_control(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
    ...
    try:
        storm_control = await aiomeraki.switch.getNetworkSwitchStormControl(network['id'])
    except meraki.exceptions.AsyncAPIError as e:
        pp(f'[bold magenta]Meraki AIO API Error (OrgID "{ org_id }", OrgName "{ org_name }"): \n { e }')
    except Exception as e:
        pp(f'[bold magenta]Some other ERROR: {e}') 
    try:
        if storm_control['broadcastThreshold'] < 100 and storm_control['multicastThreshold'] < 100 and storm_control['unknownUnicastThreshold'] < 100:
            ...
            pp(f"[yellow]Storm-control is disabled for network {network['name']}. Best practices suggest a limit should be configured.")
...
async def async_check_network_firmware(aiomeraki: meraki.aio.AsyncDashboardAPI, network: dict):
    """
...
async def async_check_org_admins(aiomeraki: meraki.aio.AsyncDashboardAPI):
    ...
    for admin in org_admins:
        results['org_settings']['users'][admin['id']] = {'email': admin['email'], 'name': admin['name'], '2fa': admin['twoFactorAuthEnabled'], 'api_calls': 0, 'using_v0': False}
    ...
    if results['org_settings']['more_than_one_admin'] == True and results['org_settings']['missing_2fa'] == []:
        results['org_settings']['is_ok'] = True
    else:
        results['org_settings']['is_ok'] = False
    # Check API access
    check_api_calls_tasks = [async_check_api_calls(aiomeraki, admin['id']) for admin in org_admins]
    ...
    pp(f"API access usage: {results['org_settings']['api_calls']} API calls during the last week.")


async def async_check_api_calls (aiomeraki: meraki.aio.AsyncDashboardAPI, org_admin: str):
    try:
        api_requests = await aiomeraki.organizations.getOrganizationApiRequests(org_id, adminId=org_admin, timespan=7*86400, perPage=1000, total_pages=10)
    except meraki.exceptions.AsyncAPIError as e:
        pp(f'[bold magenta]Meraki AIO API Error (OrgID "{ org_id }", OrgName "{ org_name }"): \n { e }')
    except Exception as e:
        pp(f'[bold magenta]Some other ERROR: {e}')         
...
def generate_excel_report(results: dict) -> None:
    ...
    sheet["B5"] = "Cisco Meraki is an amazing cloud-managed IT solution, simplifying network, security, security cameras, and IoT infrastructure."
    sheet["B6"] = "However, even the most intelligent AI/ML-driven solution is still vulnerable to users misconfiguring various options (usually without reading the documentation)."
    ...
    sheet["C16"] =      f"The threshold is set to {thresholds['5G Channel Utilization']}%. APs with utilization above this threshold for many occurrences (10+) may be experiencing RF issues."
    sheet["B17"] = "4. Firmware Upgrade - This tab presents the firmware status for every network. Highlighting networks that require a firmware upgrade."       
    sheet["B18"] = "5. RF profiles - This tab presents the (non-default) RF profiles for every network."
    sheet["C19"] =      f"Minimum Tx power: Setting the minimum Tx power too high, might result in wireless APs interfering with each other, as they are not allowed to decrease their power. The threshold is set to {thresholds['5G Min TX Power']} dBm."
    ...
    sheet["C21"] =      f"Channel Width: Depending on local regulation and wireless AP density, there is a limited number of channels that can be used. In most deployments, channel width of more than {thresholds['5G Max Channel Width']}MHz might cause interference between the wireless APs."
    ...
    sheet["C25"] =      f"Ports with collisions: It's 2022.. we shouldn't be seeing hubs or collisions on our network. Ports with more than 0 collisions will appear here."
...
async def main():
    async with meraki.aio.AsyncDashboardAPI(
        ...
        #
        # General checks
        check_network_health_tasks = [async_check_network_health_alerts(aiomeraki, net) for net in networks]
        for task in asyncio.as_completed(check_network_health_tasks):
            await task
        check_firmware_tasks = [async_check_network_firmware(aiomeraki, net) for net in networks]
        for task in asyncio.as_completed(check_firmware_tasks):
            await task
        #
        # Wired checks
        check_switch_port_tasks = [async_check_switches_port_counters(aiomeraki, net) for net in networks if "switch" in net['productTypes']]
        for task in asyncio.as_completed(check_switch_port_tasks):
            await task
        check_switch_stp_tasks = [async_check_switch_stp(aiomeraki, net) for net in networks if "switch" in net['productTypes']]
        for task in asyncio.as_completed(check_switch_stp_tasks):
            await task
        check_switch_mtu_tasks = [async_check_switch_mtu(aiomeraki, net) for net in networks if "switch" in net['productTypes']]
        for task in asyncio.as_completed(check_switch_mtu_tasks):
            await task
        check_switch_storm_control_tasks = [async_check_switch_storm_control(aiomeraki, net) for net in networks if "switch" in net['productTypes']]
        for task in asyncio.as_completed(check_switch_storm_control_tasks):
            await task                                  
        #
        # Wireless checks
        check_channel_utilization_tasks = [async_check_wifi_channel_utilization(aiomeraki, net) for net in networks if "wireless" in net['productTypes']]
        for task in asyncio.as_completed(check_channel_utilization_tasks):
            await task
        check_rf_profiles_tasks = [async_check_wifi_rf_profiles(aiomeraki, net) for net in networks if "wireless" in net['productTypes']]
        for task in asyncio.as_completed(check_rf_profiles_tasks):
            await task
        check_ssid_amount_tasks = [async_check_wifi_ssid_amount(aiomeraki, net) for net in networks if "wireless" in net['productTypes']]
